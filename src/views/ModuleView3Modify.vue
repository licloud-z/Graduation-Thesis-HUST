<template>
  <div v-if="this.is_changed == 0">
    ...
    <div>
        文件内容
        <textarea v-model="this.file_content" placeholder="请输入要写入文件的内容"></textarea>
    </div>
  ...
  <div v-else-if="this.is_changed == 1">
    ...
    <div v-else-if="this.page == this.file_level + 1">
      ...
      <!-- <vue-chart 
        ...
        :Mode=1
      /> -->
      <div class="msg-txt">向文件中写入的字符数为：</div>
    ...
    <div v-else>
      ...
      <div>修改文件前，空闲inode数量: {{ this.inode_number_before }}</div>
      <div>修改文件后，空闲inode数量：{{ this.inode_number_later }}</div>
      <div>可以看出，inode不变</div>
      <div>修改文件前，空闲block数量: {{ this.block_number_before }}</div>
      <div>修改文件后，空闲block数量：{{ this.block_number_later }}</div>
      <div>可以看出，block数量会发生对应的改变</div>
    </div>
...
export default {
  data (){
    return {
      ...
      file_name: 'a.txt',
      file_content: '身患绝症的云天明买下一颗星星送给暗恋着的大学同学程心，而程心因参与（PIA）向三体舰队发射探测器的工作，却想让航天专业背景的他放弃安乐死，作为被执行人将大脑捐献给阶梯计划。与三体文明的战争使人类首次看到了宇宙黑暗的真相，地球文明因为黑暗森林打击的存在如临大敌，不敢在太空中暴露自己。在零道德的宇宙中发起黑暗战役的战舰被诱导返航，却受到有道德的地球文明审判。',
      file_inode: 0,
      page: 0,
      is_changed: 0,
      file_level: 3,
  ...
  watch: {
    is_changed (){
      ...
      try {
        ...
        // let workprocess = exec(cmd);
        fs.writeFileSync(this.file_fullpath, this.file_content);
      }
      catch(err){
        console.log('modify file error');
      }
      // 获取修改后的文件系统信息
      try {
        let cmd = 'df -i /dev/sda5';
